    ...
    for(int i=0;i<h;i++)
    {
        for(int j=0;j<w;j++)
        {
         ...
         else if (i== h-1 && j== 0) //lower left coorner.
           {
                ...
                gy=((image[i][j].rgbtBlue * 0)+(image[i-1][j].rgbtBlue*(-2))+(image[i+1][j].rgbtBlue*(0))+(image[i][j+1].rgbtBlue*0) + (image[i-1][j+1].rgbtBlue*(-1))+(image[i+1][j+1].rgbtBlue*1) + (image[i][j-1].rgbtBlue*(0)) + (image[i-1][j-1].rgbtBlue*(-1))+(image[i+1][j-1].rgbtBlue*(1)));
                image[i][j].rgbtBlue = sqrt(((gx*gx)+(gy*gy)));
