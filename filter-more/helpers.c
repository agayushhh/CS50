...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for(int i=0;i<h;i++)
    {
        ...
        {
                gx=(image[i][j].rgbtRed+image[i-1][j].rgbtRed+image[i+1][j].rgbtRed+image[i][j+1].rgbtRed + image[i-1][j+1].rgbtRed+image[i+1][j+1].rgbtRed + image[i][j-1].rgbtRed + image[i-1][j-1].rgbtRed+image[i+1][j-1].rgbtRed);
