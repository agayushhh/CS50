    ...
    for(int i=0;i<h;i++)
    {
        for(int j=0;j<w;j++)
        {
            if(i==0 && j==0)
             ...
             {
                red=(image[i][j].rgbtRed+image[i+1][j].rgbtRed+image[i][j+1].rgbtRed+image[i+1][j+1].rgbtRed + image[i-1][j].rgbtRed+image[i-1][j+1].rgbtRed)/6;
                Green=(image[i][j].rgbtGreen + image[i+1][j].rgbtGreen +image[i][j+1].rgbtGreen +image[i+1][j+1].rgbtGreen +image[i-1][j+1].rgbtGreen +image[i-1][j].rgbtGreen)/6;
                Blue =(image[i][j].rgbtBlue+image[i+1][j].rgbtBlue+image[i][j+1].rgbtBlue+image[i+1][j+1].rgbtBlue + image[i-1][j].rgbtBlue +image[i-1][j+1].rgbtBlue)/6;
                image[i][j].rgbtRed = red;
                image[i][j].rgbtGreen = Green;
                image[i][j].rgbtBlue = Blue;
             }
